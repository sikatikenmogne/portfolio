...
import { getPostsByTag, getAllTags } from '@/lib/content-blog';
...
export async function generateMetadata({ params }) {
  ...
  const title = `Posts about ${decodedTag} - Blog | ${fullName}`;
  const description = `Discover all my articles about ${decodedTag}: tutorials, guides and insights.`;
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${siteUrl}/en/blog/tag/${tag}`,
      languages: {
    ...
    openGraph: {
      ...
      images: [{ url: ogImage, width: 1200, height: 630, alt: `Posts about ${decodedTag}` }],
      locale: 'en_US',
...
export default async function BlogTagPage({ params }) {
  const { tag } = params;
  const decodedTag = decodeURIComponent(tag);
  const posts = getPostsByTag(decodedTag).filter((post) => post.lang === 'en');
